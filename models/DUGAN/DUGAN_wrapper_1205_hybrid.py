...
class FrequencyAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        # Use rfft2 for real-valued input
...
class DownBlock(nn.Module):
    def __init__(self, input_channels, filters, downsample=True):
        ...
        # Standard DownBlock (No Attention) to preserve features
...
class UpBlock(nn.Module):
    ...
    def forward(self, x, up):
        ...
        # Apply Spatial Attention to refine the reconstructed features
...
class UNet(nn.Module):
    def __init__(self, repeat_num, use_tanh=False, use_sigmoid=False, skip_connection=True, use_discriminator=True,
        ...
        # Bottleneck: Frequency Attention + Transformer
    ...
    def forward(self, input):
        ...
        # 1. Frequency Attention: "What" to focus on (Global Frequency)
        x = self.freq_att(x)
        # 2. Transformer: Global Spatial Context
        ...
        for (up_block, res) in zip(self.up_blocks, residuals[:-1][::-1]):
            x = up_block(x, res) # Includes Spatial Attention
