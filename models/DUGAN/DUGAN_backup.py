...
class DUGAN(TrainTask):
    ...
    def build_options():
        ...
        # MoE Options
        parser.add_argument("--moe_phase", default=0, type=int, help="0: Baseline, 1: Train D_noise, 2: Train Gate")
        parser.add_argument("--lambda_noise_fixed", default=0.3, type=float, help="Fixed weight for D_noise in Phase 1")
        parser.add_argument("--lambda_gate_balance", default=0.01, type=float, help="Weight for gate load balancing loss")
        parser.add_argument("--lambda_noise_reg", default=1.0, type=float, help="Weight for noise regression loss")
        parser.add_argument("--num_experts", default=3, type=int, help="Number of experts (img, grad, noise)")
    ...
    def set_model(self):
        ...
        if hasattr(opt, 'use_hybrid_wrapper') and opt.use_hybrid_wrapper:
            print("Using Hybrid Attention Wrapper (Freq Att + Spatial Att)...")
            from .DUGAN_wrapper_hybrid import UNet as HybridUNet
            img_discriminator = HybridUNet(repeat_num=opt.repeat_num, use_discriminator=True, conv_dim=64, use_sigmoid=False)
        else:
        ...
        # MoE Components
        self.noise_discriminator = NoiseDiscriminator(in_channels=3, base_channels=64).cuda()
        self.noise_discriminator = turn_on_spectral_norm(self.noise_discriminator)
        self.noise_d_optimizer = torch.optim.Adam(self.noise_discriminator.parameters(), opt.d_lr)
        
        self.gating_network = GatingNetwork(bottleneck_channels=512, hidden_dim=128, num_experts=opt.num_experts).cuda()
        self.gate_optimizer = torch.optim.Adam(self.gating_network.parameters(), opt.g_lr)
        
        self.logger.modules.extend([self.noise_discriminator, self.noise_d_optimizer, self.gating_network, self.gate_optimizer])
    ...
    def train_noise_discriminator(self, y_hat, x_noisy, y_ref, noise_level_map):
        """
        Train D_noise.
        """
        ...
        # Fake pass
        adv_map_fake, noise_map_fake, bottleneck_fake = self.noise_discriminator(x_noisy, y_hat.detach(), noise_level_map)
        
        # Real pass (if y_ref exists)
        adv_map_real, noise_map_real, bottleneck_real = self.noise_discriminator(x_noisy, y_ref, noise_level_map)
        
        # Adversarial Loss (LSGAN)
        d_loss = self.gan_metric(adv_map_real, 1.) + self.gan_metric(adv_map_fake, 0.)
        
        # Noise Regression Loss
        noise_reg_loss = F.l1_loss(noise_map_fake, noise_level_map)
        
        total_d_loss = d_loss + self.opt.lambda_noise_reg * noise_reg_loss
        
        total_d_loss.backward()
        self.noise_d_optimizer.step()
        
        return d_loss.item(), noise_reg_loss.item()
    ...
    def train(self, inputs, n_iter):
        ...
        # Train Noise Discriminator (Phase 1 & 2)
        if opt.moe_phase >= 1:
            d_noise_loss, noise_reg_loss = self.train_noise_discriminator(gen_full_dose, low_dose, full_dose, noise_level_map)
            msg_dict.update({'loss/d_noise': d_noise_loss, 'loss/noise_reg': noise_reg_loss})
        ...
        if n_iter % opt.d_iter == 0:
            ...
            # Noise Loss
            loss_g_noise = 0.
            bottleneck_fake = None
            if opt.moe_phase >= 1:
                adv_map_fake, noise_map_fake, bottleneck_fake = self.noise_discriminator(low_dose, gen_full_dose, noise_level_map)
                loss_g_noise = self.gan_metric(adv_map_fake, 1.)
                l_noise_reg_g = F.l1_loss(noise_map_fake, noise_level_map)
            ...
            # Combine Losses
            l_recon = img_gen_loss * opt.img_gen_loss_weight + \
                      pix_loss * opt.pix_loss_weight + \
                      grad_loss * opt.grad_loss_weight
            
            # Note: Original code added img_gen_loss here.
            # But in MoE we might want to weight it dynamically.
            # Original: total_loss += img_gen_loss * opt.img_gen_loss_weight ...
            
            # Let's reconstruct the logic:
            # If Phase 0: Use original logic.
            # If Phase 1/2: Use MoE logic for Adv losses.
            ...
            if opt.moe_phase == 0:
                ...
                # Base recon (Pixel + Grad L1)
                # Note: Original included img_gen_loss in "total_loss" calculation line 193.
                # We separate Adv losses.
                total_loss = pix_loss * opt.pix_loss_weight + grad_loss * opt.grad_loss_weight
                ...
                if opt.moe_phase == 2:
                    ...
                    alphas = self.gating_network(gen_full_dose, residual_abs, bottleneck_fake)
                    mean_alphas = alphas.mean(dim=0)
                    alpha_img, alpha_grad, alpha_noise = mean_alphas[0], mean_alphas[1], mean_alphas[2]
                    
                    adv_loss_total = alpha_img * img_gen_loss * opt.img_gen_loss_weight + \
                                     alpha_grad * grad_gen_loss * opt.grad_gen_loss_weight + \
                                     alpha_noise * loss_g_noise
                    
                    balance_loss = self.gating_network.balance_loss(alphas)
                    
                    msg_dict.update({
                        'gate/alpha_img': alpha_img.item(),
                        'gate/alpha_grad': alpha_grad.item(),
                        'gate/alpha_noise': alpha_noise.item(),
                        'loss/gate_balance': balance_loss.item()
                    })
                else: # Phase 1
                    adv_loss_total = img_gen_loss * opt.img_gen_loss_weight + \
                                     grad_gen_loss * opt.grad_gen_loss_weight + \
                                     loss_g_noise * opt.lambda_noise_fixed
                
                total_loss += adv_loss_total
                total_loss += opt.lambda_noise_reg * l_noise_reg_g
                if opt.moe_phase == 2:
                    total_loss += opt.lambda_gate_balance * balance_loss
            ...
            msg_dict.update({
                ...
                'loss/l1': l1_loss,
                'loss/grad': grad_loss,
                'loss/g_noise': loss_g_noise if opt.moe_phase >= 1 else 0.
            })
            self.logger.msg(msg_dict, n_iter)
    ...
    def generate_images(self, n_iter):
        self.generator.eval()
        if hasattr(self, 'noise_discriminator'): self.noise_discriminator.eval()
        if hasattr(self, 'gating_network'): self.gating_network.eval()
        ...
        if hasattr(self, 'noise_discriminator') and self.opt.moe_phase >= 1:
            noise_level_map = torch.abs(low_dose - full_dose)
            adv_map, noise_map, bottleneck = self.noise_discriminator(low_dose, gen_full_dose, noise_level_map)
            fake_imgs.append(adv_map.clamp(0., 1.))
            fake_imgs.append(noise_map.clamp(0., 1.))
            
            if hasattr(self, 'gating_network') and self.opt.moe_phase == 2:
                residual_abs = torch.abs(low_dose - gen_full_dose)
                alphas = self.gating_network(gen_full_dose, residual_abs, bottleneck)
                mean_alphas = alphas.mean(dim=0)
                print(f"Iter {n_iter} Alphas: Img={mean_alphas[0]:.3f}, Grad={mean_alphas[1]:.3f}, Noise={mean_alphas[2]:.3f}")

        fake_imgs = torch.stack(fake_imgs).transpose(1, 0).reshape((-1, ch, w, h))
        self.logger.save_image(torchvision.utils.make_grid(fake_imgs, nrow=len(fake_imgs)//bs), n_iter, 'test')
